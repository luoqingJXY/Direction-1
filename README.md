...
```text
L = (W, B, C, U, A, Q, P, M, Y, R)
```

下一项真实外部或内部发生记为 `ε`，全部变化统一受以下关系约束：
...
| 内容 | 可查看的具体材料 |
|---|---|
| [实验档案](experiments/README.md) | 两次实验的关系、有效性边界和完整索引 |
| [第一次实验](experiments/01_第一次实验/README.md) | 272个神经元、864条路径的小闭环报告，以及逐信号、逐神经元、逐路径、世界实体、预测和睡眠观测记录 |
| [第二次实验](experiments/02_第二次实验/README.md) | 双电脑结构、视觉和听觉规格、动作器官、102,400,000个神经元位置、2,662,400,000个局部路径槽位、本能结构、实际启动和故障证据 |
| [人工出生结构源码](source/artificial_dna/README.md) | 神经组织地址、器官入口、固定路径、局部可塑空间、视觉通道、动作通道和生命状态本能的实际生成代码 |
| [乙电脑外围源码](source/peripherals/host_b/README.md) | Minecraft画面、听觉、局域网、教学、键鼠执行和声音输出程序 |
| [其他公开源码](source/README.md) | 记忆缓存边界、控制台、睡眠、器官输入输出、声音器官和硬件工具 |
| [原始设计材料](history/README.md) | 理论形成稿和两份实际设计文档 |

第一次实验的详细数据文件位于 `experiments/01_第一次实验/records`，包括 `summary.json`、`structure.json`、`timeline.csv`、`signals.csv`、`neurons.csv`、`paths.csv`、`sleep_settlement.csv`、`path_changes.csv` 和世界实体记录。第二次实验的大型个体数组没有上传，但其数组形状、路径数量、字节规模和编译状态已经保存在 `experiments/02_第二次实验/manifests`。

## 仓库范围

这个公开仓库现在包含理论、实验档案、人工出生结构和除大脑本体外的工程代码。仅不包含：

- 正式大脑传播运行器、状态变化执行核心和显卡全脑计算后端；
- 大型人工生命状态、神经元数组与路径数组；
- 乙电脑二进制安装包和可重新生成的开发环境；
- 本地网络地址、设备配置和个人环境信息；
- 尚未确认的实验参数或已经撤销的机制。
